...
predict_names_task = SimpleHttpOperator(
    ...
    method='POST',
    http_conn_id='gender_api_AnggiSetiawanDC',
...
create_table_task = PostgresOperator(
    ...
    postgres_conn_id='pg_conn_Anggi',
    sql="""
    CREATE TABLE IF NOT EXISTS Anggi_gender_name_prediction (
        input JSONB,
        details JSONB,
...
def load_predictions_to_postgres(**kwargs):
    ...
    pg_hook = PostgresHook(postgres_conn_id='pg_conn_anggi')
    for prediction in predictions:
        ...
        pg_hook.run("""
            INSERT INTO Anggi_gender_name_prediction (input, details, result_found, first_name, probability, gender)
            VALUES (%s, %s, %s, %s, %s, %s);
